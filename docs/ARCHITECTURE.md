...
Inventory owns Warehouse-level current on-hand state for a Tenant/Warehouse/SKU cell. Its PostgreSQL-specific atomic upsert is isolated in Inventory infrastructure; Receiving does not access Inventory repositories or entities. InventoryBalance is current mutable state, while a future Inventory Ledger will provide immutable movement history. Bin/location distribution, Putaway, and reservation remain later workflows.
